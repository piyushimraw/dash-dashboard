...
vi.mock('./table-pagination', () => ({
  TablePagination: ({ table }: any) => (
    <div data-testid="pagination">Page {table.getState().pagination.pageIndex}</div>
  ),
}));
...
async function mockDesktop(value: boolean) {
  ...
  const mockedUseIsDesktop = mod.useIsDesktop as MockedFunction<typeof mod.useIsDesktop>;
