...
import type { NavigationGroup, NavigationItem } from "@dash/mfe-types";
...
// Static menu items that are not part of MFEs
...
function convertNavigationGroupToMenuItem(group: NavigationGroup): MenuItem {
  const IconComponent = group.icon;
  return {
    label: group.label,
    icon: <IconComponent className="h-4 w-4" aria-hidden="true" />,
    children: group.items.map((item: NavigationItem) => ({
      label: item.label,
      icon: item.icon ? (
...
export function Sidebar({ onLogout, isOpen, onClose }: SidebarProps) {
  // Get dynamic navigation from MFE registry
  const menuItems = useMemo(() => {
    const dynamicNavGroups = mfeRegistry.getAllNavigation();
    const dynamicMenuItems = dynamicNavGroups.map(
      convertNavigationGroupToMenuItem
    );

    // Add AAO to Counter Functions (from dynamic navigation)
    const counterFunctions = dynamicMenuItems.find(
      (item) => item.label === "Counter Functions"
    );
    if (counterFunctions && counterFunctions.children) {
      // Add AAO if not already present
      if (!counterFunctions.children.some((c) => c.label === "AAO")) {
        counterFunctions.children.push({ label: "AAO", pathname: "/aao" });
      }
    }

    // Combine dynamic and static menu items
    return [...dynamicMenuItems, ...staticMenuItems];
  }, []);
