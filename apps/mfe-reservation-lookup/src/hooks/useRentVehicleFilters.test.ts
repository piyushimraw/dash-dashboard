...
describe('useRentVehicleFilters', () => {
  it('should initialize with full data and default values', () => {
    const { result } = renderHook(() => useRentVehicleFilters(rentedVehiclesMock));
  ...
  it('should filter by status = Confirmed', () => {
    ...
    expect(result.current.filteredData.every((item) => item.resStatus === 'Confirmed')).toBe(true);
  });
  ...
  it('should filter by arrival location DEL', () => {
    ...
    expect(result.current.filteredData).toHaveLength(2);
    expect(result.current.filteredData.every((item) => item.arrivalLocation === 'DEL')).toBe(true);
  });
  ...
  it('should filter by date range (Jan 2025 only)', () => {
    ...
    expect(result.current.filteredData.map((i) => i.id)).toEqual(['1', '2', '4']);
  });
